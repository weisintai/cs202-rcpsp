...
def test_enforce_runtime_limit_can_use_wall_clock_observation() -> None:
    result = SolveResult(
        instance_name="PSP48",
        status="feasible",
        schedule=Schedule(start_times=(0, 1), makespan=1),
    ...
    limited = _enforce_runtime_limit(
        result,
        time_limit=1.0,
        observed_runtime_seconds=1.2,
        runtime_basis="wall",
    )

    assert limited.status == "unknown"
    assert limited.schedule is None
    assert limited.runtime_seconds == 1.2
    assert limited.metadata["solver_runtime_seconds"] == 0.8
    assert limited.metadata["runtime_basis"] == "wall"
